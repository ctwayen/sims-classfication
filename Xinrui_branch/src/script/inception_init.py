...
class Trainer():
    def __init__(self,model,train_set,test_set,opts):
        ...
        self.test_loader = torch.utils.data.DataLoader(dataset=test_set,
                                                    batch_size=opts['batch_size'],
                                                    shuffle=False)
        self.tb = SummaryWriter(log_dir='./runs/init/')
        self.best_loss = 1e10
    ...
    def test(self,epoch):
            ...
            if np.mean(self.test_loss) < self.best_loss:
                ...
                torch.save(self.model.state_dict(), './model_weights/inception_after_init.pt')

dataset = classification(paths, labels, size=(512, 512))
print(len(dataset))
train_set, val_set = torch.utils.data.random_split(dataset, [4600, 1454])
